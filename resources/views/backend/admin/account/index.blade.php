        ...
        <div class="card-body">
            <div class="heading-layout1">
                ...
                @if(check_menu_button('accounts','create'))
                   <a href="{{route('admin.account.create')}}" class="btn-fill-lg bg-blue-dark btn-hover-yellow"><i class="fas fa-plus"></i> Add Accounts</a>
                @endif
            </div>
            {{-- 
            <form class="mg-b-20">
                        ...
                        <tr>
                            ...
                            <td>
                                    ...
                                    <span style="color:green;">
                                        {{ ucfirst($item->mobile_payment_types->name) }}
                                    </span>
                                        @else
                                        {{ ucfirst($item->payment_methods->name) }}
                                    @endif
                              
                            </td>
                            <td>{{ ucfirst($item->account_name) }}</td>
                            <td>{{ ucfirst($item->account_no) }}</td>
                            <td>{{ ucfirst($item->bank_name) }}</td>
                            <td>
                                {{ ucfirst($item->bank_address) }}
                            </td>
                            <td>
                                <div class="dropdown">
                                    ...
                                    <div class="dropdown-menu dropdown-menu-right">
                                       @if(check_menu_button('accounts','show'))
                                        <a class="dropdown-item" href="{{route('admin.account.show',$item->id)}}"><i class="fas fa-eye text-dark-pastel-green"></i>View</a>
                                        @endif
                                        @if(check_menu_button('accounts','edit'))
                                        <a class="dropdown-item" href="{{route('admin.account.edit',$item->id)}}"><i class="fas fa-edit text-orange-peel"></i>Edit</a>
                                        @endif
                                        @if(check_menu_button('accounts','delete'))
                                        <a class="dropdown-item delete" href="#" data-toggle="modal" data-url="{{route('admin.account.delete',$item->id)}}"  data-toggle="modal" data-target="#myModal"><i class="fas fa-times text-orange-red"></i>Delete</a>
                                        @endif
                                    </div>
                                </div>
                            </td>
