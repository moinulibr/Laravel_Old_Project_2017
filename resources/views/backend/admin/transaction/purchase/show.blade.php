                    ...
                    <div class="info-table table-responsive" id="toprint" >
                        ...
                        <div class="order-addresses">
                            <div class="billing-address">
                                <h3>Supplier:</h3>
                                <p>
                                {{ $final_purchase->users->name }},</br>
                                {{ $final_purchase->users->company_name }},</br>
                                {{ $final_purchase->users->address }}</br>
                                Mobile: {{ $final_purchase->users->phone }}</br>
                                Mobile 2: {{ $final_purchase->users->phone_2?'$final_purchase->users->phone_2':'NILL' }}</br>
                                {{ $final_purchase->users->email }}
                            </p>
                            </div>
                            ...
                            <table class="order-info">
                                <tr>
                                    <th>Reference No:</th>
                                    <td>  {{ $final_purchase->substr_invoice() }}</td>
                                </tr>
                                <tr>
                                    <th>Purchase Date:</th>
                                    <td> {{ date('d-m-Y',strtotime($final_purchase->purchase_date)) }}</td>
                                </tr>
                                <tr>
                                    <th>Payment Status:</th>
                                    <td>
                                        @if ($final_purchase->payment_status == NULL || $final_purchase->payment_status == 'Unpaid')
                        ...
                        <table class="order-items">
                            ...
                            <tfoot style="text-align: right;">
                                <tr class="order-discount">
                                    ...
                                    <td colspan="1">{{$final_purchase->sub_total}}</td>
                                </tr>
                                <tr class="order-discount">
                                    <th colspan="4">Fee:</th>
                                    <td colspan="1">{{$final_purchase->fee}}</td>
                                </tr>
                                <tr class="order-discount">
                                    <th colspan="4">Discount:</th>
                                    <td colspan="1">{{$final_purchase->discount}}</td>
                                </tr>
                                <!-- end order_discount -->
                                <tr class="order-total">
                                    <th colspan="4">Payable Amount:</th>
                                    <td colspan="1">{{$final_purchase->final_total}}</td>
                                </tr>
                                <tr class="pos_cash-tendered">
                                    <th colspan="4">Paid Amount:</th>
                                    <td colspan="1">{{$final_purchase->paid_total}}</td>
                                </tr>
                                <tr class="pos_cash-change">
                                    <th colspan="4">Due Amount:</th>
                                    <td colspan="1">{{ $final_purchase->final_total - $final_purchase->paid_total}}</td>
                                </tr>
                            </tfoot>
                        </table>
